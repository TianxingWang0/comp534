...
int GenerateMove(int color, Board *b, int depth) {
  ...
  if (color == X_BLACK) {  // for X, maximize the minimum
    best_move = MaxValue(b, depth, true).m;
  } else {                 // for O, minimize the maximum
    best_move = MinValue(b, depth, true).m;
  }
...
Value MaxValue(Board* bptr, int depth, bool pre_moved) {
  if (depth) {
      ...
      for (int col = 8; thisrow && (col >= 1); col--) {
        if (thisrow & COL8) {
          ...
          if (FlipDisks(m, bptr, X_BLACK, 0, 0) > 0) {
            ...
            // PrintBoard(&b);
            max_value.calc_max({m, MinValue(&b, depth, true).value});  // make move at this level
          }
        }
        thisrow >>= 1;
      }
    ...
    if (pre_moved) {
      return MinValue(bptr, depth, false);  // no legal move this level
    }
  }
...
Value MinValue(Board* bptr, int depth, bool pre_moved) {
  if (depth) {
      ...
      for (int col = 8; thisrow && (col >= 1); col--) {
        if (thisrow & COL8) {
          ...
          if (FlipDisks(m, bptr, O_WHITE, 0, 0) > 0) {
            ...
            // printf("X generate move\n");
            // PrintBoard(&b);
            min_value.calc_min({m, MaxValue(&b, depth, true).value});  // make move at this level
          }
        }
        thisrow >>= 1;
      }
    ...
    if (pre_moved) {
      return MaxValue(bptr, depth, false);  // no legal move this level
    }
  }
  return {{0, 0}, Evaluate(bptr)};
}
